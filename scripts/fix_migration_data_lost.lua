                ...
                if success and str ~= nil and #str > 0 then
                    ...
                    TheSim:GetPersistentString("session/" .. TheWorld.meta.session_identifier .. "/userdata", function(get, user_table)
                        if get and user_table then
                            ...
                            if status and user_table ~= nil and GetTableSize(user_table) > 0 then
                                user = user_table
                            end
                        end
                    end)
                    if user[num - 1] == nil then
                        user[num - 1] = {}
                    end
...
getmetatable(TheNet).__index["TruncateSnapshots"] = function(self, ...)
    ...
    if user[TheNet:GetCurrentSnapshot()] then
        ...
        for _, v in pairs(user[TheNet:GetCurrentSnapshot()]) do
            local user_session_inner = TheNet:GetUserSessionFile(TheWorld.meta.session_identifier, v)
            if user_session_inner ~= nil then
                TheNet:DeserializeUserSession(user_session_inner, function(success, str)
                    if success and str ~= nil and #str > 0 and string.find(str, "KLEI") ~= nil and string.find(str, "KLEI") < 10 then
                        pstr = true
                    end
                end)
                if pstr then

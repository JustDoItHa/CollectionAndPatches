...
local function GetLoiterAnchor(inst)
    ...
    elseif
        inst.components.knownlocations:GetLocation("herd") ~= nil and
            inst:GetDistanceSqToPoint(inst.components.knownlocations:GetLocation("herd")) <
                LOITER_ANCHOR_HERD_DIST * LOITER_ANCHOR_HERD_DIST
     then
        inst.components.knownlocations:RememberLocation(
            "loiteranchor",
            inst.components.knownlocations:GetLocation("herd")
        )
    elseif
        inst:GetDistanceSqToPoint(inst.components.knownlocations:GetLocation("loiteranchor")) >
            LOITER_ANCHOR_RESET_DIST * LOITER_ANCHOR_RESET_DIST
     then
        inst.components.knownlocations:RememberLocation("loiteranchor", inst:GetPosition())
    end
...
local function TryBeginLoiterState(inst)
    ...
    if
        (herd and herd.components.mood and herd.components.mood:IsInMood()) or
            (inst.components.mood and inst.components.mood:IsInMood())
     then
        return false
    end
...
local function TryBeginGreetingState(inst)
    ...
    if
        inst.components.domesticatable ~= nil and inst.components.domesticatable:GetDomestication() > 0.0 and
            GetGreetTarget(inst) ~= nil
     then
...
local function GetLeader(inst)
    local leader =
        inst.components.follower ~= nil and inst.components.follower.leader or
        inst.ownerlist and GetTheWorldPlayerById(inst.ownerlist.master) or
        nil
...
local function KeepAttackingAction(inst)
    ...
    return (inst.components.follower.leader ~= nil and inst:IsNear(inst.components.follower.leader, KEEP_ATTACKING_DIST)) or
        (leader ~= nil and inst:IsNear(leader, KEEP_ATTACKING_DIST))
end

local function StartAttackingCondition(inst)
    local leader = GetLeader(inst)
    return (inst.components.follower.leader ~= nil and inst.components.follower.leader.sg ~= nil and
        inst.components.follower.leader.sg:HasStateTag("attack")) or
        (leader ~= nil and leader.sg ~= nil and leader.sg:HasStateTag("attack"))
end
...
local function IsHostileAttacker(guy, inst)
    ...
    return guy.components.combat and
        ((leader ~= nil and guy.components.combat.target == leader) or
            (inst.components.follower.leader ~= nil and guy.components.combat.target == inst.components.follower.leader))
end
...
function BeefaloBrain:OnStart()
    local root =
        PriorityNode(
        {
            ...
            IfNode(
                function()
                    return StartAttackingCondition(self.inst)
                end,
                "attack",
                WhileNode(
                    function()
                        return KeepAttackingAction(self.inst)
                    end,
                    "keep attacking",
            ...
            IfNode(
                function()
                    return StartAttackerCondition(self.inst)
                end,
                "attacker",
                WhileNode(
                    function()
                        return KeepAttackerAction(self.inst)
                    end,
                    "keep attacker",

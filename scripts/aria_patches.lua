...
if GetModConfigData("turn_off_aria_transfer_replicate_function") then
    AddPrefabPostInit("aria_transfer", function(inst)
        ...
        inst.transfer = function(inst, ...)
            local item = inst.components.container:GetItemInSlot(5)
            if item ~= nil and item.components.equippable == nil and item.components.container == nil and item.components.unwrappable == nil
                and ((inst.components.container:HasItemWithTag("aria_stone", 8) 
                and not item:HasTag("aria_stone")) or inst.components.container:HasItemWithTag("aria_stone", 9))
                and inst.replica.container:IsFull() 
            then
                inst.components.container:DropEverything()
            end
            return old_transfer_fun and old_transfer_fun(inst, ...)
        end
        --直接覆盖 希望作者加个开关吧

...
--	"worldmapexplorer",
--}
table.insert(PrefabFiles, "globalposition_classified")
table.insert(PrefabFiles, "smoketrail")
table.insert(PrefabFiles, "globalmapicon_noproxy")
table.insert(PrefabFiles, "worldmapexplorer")
...
--}
table.insert(Assets, Asset("IMAGE", "minimap/campfire.tex"))
table.insert(Assets, Asset("ATLAS", "minimap/campfire.xml"))
table.insert(Assets, Asset("IMAGE", "images/status_bg.tex"))
table.insert(Assets, Asset("ATLAS", "images/status_bg.xml"))
table.insert(Assets, Asset("IMAGE", "images/sharelocation.tex"))
table.insert(Assets, Asset("ATLAS", "images/sharelocation.xml"))
table.insert(Assets, Asset("IMAGE", "images/unsharelocation.tex"))
table.insert(Assets, Asset("ATLAS", "images/unsharelocation.xml"))
...
if ENABLEPINGS then
    --Only request loading of ping assets if pings are enabled
...
if mode == "wilderness" and not OVERRIDEMODE then
    --by default, have different settings for wilderness
        ...
        player:ListenForEvent("setowner", function()
            ...
            if SHAREMINIMAPPROGRESS then
                ...
                elseif player ~= GLOBAL.AllPlayers[1] then
                    --The host always has the master map
...
if SHOWFIRES then
    ...
    if deluxe_campfires_installed then
        AddPrefabPostInit("deluxe_firepit", function(inst)
            FirePostInit(inst, { x = 350, y = -350 })
        end)
        AddPrefabPostInit("heat_star", function(inst)
            FirePostInit(inst, { x = 230, y = -230 })
        end)
    end
end
...
function TargetIndicator:GetAvatarAtlas(...)
    ...
    if type(self.target.userid) == "userdata" then
        --this is a globalposition_classified
...
AddClassPostConstruct("screens/playerhud", function(PlayerHud)
    ...
    function PlayerHud:AddTargetIndicator(target)
        if type(target.userid) ~= "userdata" then
            --this is a normal player target indicator
        ...
        if self.targetindicators then
            for j, w in pairs(self.targetindicators) do
                ...
                if type(w.target.userid) == "userdata" then
                    --if it's a globalposition_classified...
                    ...
                    for k, v in pairs(onscreen) do
                        --check if its userid matches an onscreen player...
...
if ENABLEPINGS then
    ReceivePing = function(player, pingtype, x, y, z)
        ...
        elseif pingtype == "clear" then
            ...
            ping.OnRemoveEntity = function(inst)
                pings[inst.GUID] = nil
            end
    ...
    HidePingWheel = function(cancel)
        if not pingwheelup or activepos == nil then
            return
        end
        pingwheelup = false
...
AddClassPostConstruct("widgets/mapwidget", function(MapWidget)
    ...
    function MapWidget:OnUpdate(dt)
        ...
        if not self.shown or pingwheelup then
            return
        end
        ...
        if SHOWPLAYERICONS then
            ...
            for k, v in pairs(GLOBAL.TheWorld.net.components.globalpositions.positions) do
                if not GLOBAL.TheFrontEnd.mutedPlayers[v.parentuserid:value()] then
                    --v.userid:value() ~= "nil" then -- this is a player's position
...
AddClassPostConstruct("screens/mapscreen", function(MapScreen)
    ...
    function MapScreen:OnBecomeInactive(...)
        ...
        if ENABLEPINGS then
            HidePingWheel(true)
        end -- consider it to be a cancellation
...
if NETWORKPLAYERPOSITIONS then
    --Don't bother unless positions are actually being networked
    ...
    function PlayerStatusScreen:DoInit(ClientObjs, ...)
        ...
        if not self.scroll_list.old_updatefn then
            -- if we haven't already patched the widgets

...
function LoadModData(id)
    ...
    if value == nil then
        print("ModDragZoomUI读取失败, 再次尝试", id)
    end
    return value
end
...
local function ShowPositionText(text)
    if not positionTextUI then
        return
    end
    positionTextUI:Show()
    positionTextUI:SetString(text)
    if positionTextUI.task then
        positionTextUI.task:Cancel()
    end
    positionTextUI.task =
        positionTextUI.inst:DoTaskInTime(
        1.5,
        function()
            positionTextUI:Hide()
...
function InitUIFollowMouse(ui)
    -- [[初始化条件判断]]
    if not (ui and ui._DragZoomUIMod and ui._DragZoomUIMod.init) then
        return
    end
    ...
    ui._DragZoomUIMod.FollowMouse = function(self, ...)
        if self.followhandler == nil then
            ...
            self._DragZoomUIMod.SetPosition(
                self,
                mousepos.x + self._DragZoomUIMod.offset_x,
                mousepos.y + self._DragZoomUIMod.offset_y,
                mousepos.z + self._DragZoomUIMod.offset_z
            )
            local worldpos = ui:GetWorldPosition()
            ShowPositionText(
                self._DragZoomUIMod.key ..
                    "\n" ..
                        GetPosStr(
                            mousepos.x + self._DragZoomUIMod.offset_x,
                            mousepos.y + self._DragZoomUIMod.offset_y,
            ...
            self.followhandler =
                ...
                function(x, y)
                    ...
                    self._DragZoomUIMod.SetPosition(
                        self,
                        loc_pos.x + self._DragZoomUIMod.offset_x,
                        loc_pos.y + self._DragZoomUIMod.offset_y,

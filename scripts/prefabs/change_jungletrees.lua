...
local builds =
{
	normal = {
		...
		normal_loot = {"log", "log", "change_coconut"},
		short_loot = {"log"},
		tall_loot = {"log", "log", "log", "change_jungletreeseed","change_coconut"},
...
local function SetNormal(inst)
	...
	if math.random() >= 0.2 then
		...
		inst.components.lootdropper:AddChanceLoot("change_coconut", 0.4)
		inst.components.lootdropper:AddChanceLoot("change_jungletreeseed", 0.4)
	end
...
local function SetTall(inst)
	...
	inst.components.lootdropper:AddChanceLoot("change_coconut", 0.6)
	inst.components.lootdropper:AddChanceLoot("change_jungletreeseed", 0.6)
	inst.components.lootdropper:AddChanceLoot("change_jungletreeseed", 0.1)
...
local function makefn(build, stage, data)
	...
	local function fn(Sim)
		...
		-- inst.MiniMapEntity:SetIcon("palmtree.tex")   --设置在地图上的图标，需要像人物一样先在modmain中先声明xml才行

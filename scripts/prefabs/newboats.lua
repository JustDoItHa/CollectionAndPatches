...
local function MakeBoat(name, radius)
    ...
    local function wood_fn()
        ...
        local data_pre = { radius = radius, max_health = max_health, item_collision_prefab = item_collision_prefab, scale = scale, boatlip = boatlip }

        inst = create_common_pre(inst, bank, build, data_pre)
        ...
        local data_pst = { bank = bank, build = build, radius = radius, max_health = max_health, item_collision_prefab = item_collision_prefab, scale = scale, boatlip = boatlip }
        inst = create_master_pst(inst, data_pst)
    ...
    return { Prefab("boat_" .. name, wood_fn, assets, prefabs),
             Prefab("boat_" .. name .. "_player_collision", function()
                 return boat_player_collision_template(radius)
             end),
             Prefab("boat_" .. name .. "_item_collision", function()
                 return boat_item_collision_template(radius)
             end),

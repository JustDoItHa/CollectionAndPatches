...
local PALMTREE_GROW_TIME =
	    {
	        ...
	        {base=8*day_time, random=4*day_time},   --normal to tall
	    }

local	    PALMTREE_CHOPS_SMALL = 3
local	    PALMTREE_CHOPS_NORMAL = 4
local	    PALMTREE_CHOPS_TALL = 5
...
SetSharedLootTable('change_palmtree_short',  {{"log", 1.0},})
SetSharedLootTable('change_palmtree_normal', {{"log", 1.0},{"log", 0.5}, })
SetSharedLootTable('change_palmtree_tall',  
{
 {"log", 1.0}, 
 {"log", 0.5}, 
 {"greengem", 0.01},
...
local function SetTall(inst)
	...
	--inst.components.lootdropper:AddChanceLoot("change_coconut", 0.167)
...
local function chop_tree(inst, chopper, chops)
	...
	if inst.components.growable and inst.components.growable.stage == 3 then
		if math.random() <= PALMTREE_COCONUT_CHANCE then
			local coconut = SpawnPrefab("change_coconut")
...
local function makefn(build, stage, data)
	...
	local function fn()
		...
		-- inst.MiniMapEntity:SetIcon("change_light.tex")

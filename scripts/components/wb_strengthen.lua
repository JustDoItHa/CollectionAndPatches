...
buffs_config['damage'] = {
  ...
  bind_fn = function (inst, level, state, config)
    ...
    inst.components.weapon.SetDamage = _.Wrap(inst.components.weapon.SetDamage, function(fn, weapon, damage, ...)
      if damage ~= nil then state.original_damage = damage end
      if type(state.original_damage) == "number" then 
        ...
        if inst.damage ~= nil then
          inst:DoTaskInTime(0, function() inst.damage = damage end) -- 覆盖魂刃，狐狸棒，光剑等升级武器显示错误
        end
      else
...
buffs_config['absorb_percent'] = {
  ...
  bind_fn = function (inst, level, state, config)
    ...
    inst.components.armor.SetAbsorption = _.Wrap(inst.components.armor.SetAbsorption, function(fn, armor, absorb_percent, ...)
      if absorb_percent ~= nil then state.original_absorb_percent = absorb_percent end
  ...
  update_fn = function (inst, level, state, config)
    ...
    if state.level_absorb_percent > 1 then state.level_absorb_percent = 0.9999 end
...
buffs_config['poison'] = {
  ...
  onattackfn = function (inst, level, state, config, attacker, target, projectile)
    level = math.floor(level * (9 / 12))
    if level > 9 then level = 9 end
    if not target.LyMagicWeaponPoisonLevel or target.LyMagicWeaponPoisonLevel < level then --------剧毒覆盖
        ...
        if not target.LyMagicWeaponPoisonFx then ---------毒气泡泡特效预留
        ...
        if target.LyMagicWeaponPoisonTask then ------------消除旧的中毒
        ...
        if not target.LyMagicWeaponPoisonTask and target.components.health and not target.components.health:IsDead() and
            not target:HasTag("wall") then -------------制造新的中毒
        ...
        if target.LyMagicWeaponPoisonStopTask then -----------消除旧的解毒延时函数
            target.LyMagicWeaponPoisonStopTask:Cancel()
            target.LyMagicWeaponPoisonStopTask = nil
        end
        target.LyMagicWeaponPoisonStopTask = target:DoTaskInTime(10, function() -----------10秒后解毒
            if target.LyMagicWeaponPoisonTask then ---------------停止中毒
                target.LyMagicWeaponPoisonTask:Cancel()
                target.LyMagicWeaponPoisonTask = nil
            end
            if target.AnimState then
            ...
            if target.LyMagicWeaponPoisonFx then ---------消除中毒泡泡特效
...
buffs_config['anti_cold'] = {
  ...
  bind_fn = function (inst, level, state, config)
    ...
    if type and type ~= SEASONS.WINTER then return end
...
buffs_config['protect'] = {
  ...
  protect_carehealth = function (owner, data)
      ...
      local cd = owner.ProtectCareHealthCd and ((600 - 50 * level) - GetTime() + owner.ProtectCareHealthCd) or 0
      ...
      if currenthealth <= 30 and cd <= 0 then
          ...
          owner:DoTaskInTime(0, function()
              owner.components.health:DoDelta(0) -----------防止显示错误
          end)
  ...
  ontakedamage = function (inst, level, state, config, damage_amount)
    ...
    for k, v in pairs(ents) do
        if v and v:IsValid() and v ~= owner and v.components.combat and v.components.health and
            ...
            v.components.combat:GetAttacked(owner, 1)
...
for buff_name, buff in pairs(buffs_config) do
  if buff.onattackfn then
    buff.bind_fn = function (inst, level, state, config)
      ...
      if inst.components.weapon then
        ...
        if weapon.__onattackfn_map == nil then weapon.__onattackfn_map = {} end
  ...
  elseif buff.ontakedamage then
    buff.bind_fn = function (inst, level, state, config)
      ...
      if inst.__onarmordamaged_map == nil then inst.__onarmordamaged_map = {} end
  ...
  elseif buff.onequipped and buff.onunequipped then
    buff.bind_fn = function (inst, level, state, config)
      ...
      if inst.__onequipped_map == nil then inst.__onequipped_map = {} end
      if inst.__onunequipped_map == nil then inst.__onunequipped_map = {} end
...
function WbStrengthen:OnLoad(data)
  if data then
    if data.do_mode ~= nil then self.do_mode = data.do_mode end
    if data.level ~= nil then self.level = data.level end
    if data.original_name ~= nil then self.original_name = data.original_name end
    if data.buffs_status ~= nil then self.buffs_status = data.buffs_status end
    if data.prize_buff_list ~= nil then self.prize_buff_list = data.prize_buff_list end
    self:SetLevel(self.level)
...
function WbStrengthen:Refresh()
  -- 脱下装备再换上，刷新状态
  if not self.inst.components.inventoryitem then return end
...
function WbStrengthen:SetLevel(level, cheating)
  ...
  if due_prize_buff_count > #self.prize_buff_list then
    ...
    for buff_name, buff in pairs(prize_buffs_config) do
      ...
      local checkhastag = buff.tags == nil or _.Every(buff.tags, function (tag) return self.inst:HasTag(tag) end) -- 检查是否有标签
      local checkexistence = _.Includes(self.prize_buff_list, buff_name) ~= true -- 检查有没有已经存在
      local possible = 0.05 -- 26%概率,根据古典概率模型计算
      if cheating then possible = 1 end
      if checklevel and checkincrease and checkhastag and checkexistence and math.random() < possible then 
        ...
        if not cheating then break end
      end
    end
...
function WbStrengthen:DoSay(player, do_mode, do_level, is_success)
  if not player or not player.components.talker then return end

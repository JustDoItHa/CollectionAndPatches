...
local Clientpause_keyhandler = Class(function(self, inst)
	self.inst = inst
	self.paused = false
	self.ignore_ = false
	...
	self.inst:ListenForEvent( "ignore", function(inst)
		ignore( inst.components.clientpause_keyhandler )
	end)
end)

function Clientpause_keyhandler:StartIgnoring()
	self.ignore_event:push()
end

function Clientpause_keyhandler:StopIgnoring()
	self.ignore_event:push()
end

function Clientpause_keyhandler:OnRawKey(key, down)
...
function Clientpause_keyhandler:AddActionListener(Namespace, Key, Action, Event)
	if Event == nil then

...
function Save_Beefalo:SaveBeefalo()
        ...
        for _, ent in pairs(ents) do
            if
                ent.components.domesticatable and ent.components.rideable and
                    ...
                    ent.ownerlist.master == self.inst.userid
             then
                beefalo = ent
                break
            end
    ...
    if beefalo ~= nil and beefalo.ownerlist and beefalo.ownerlist.master == self.inst.userid then
        --确定要保存的牛实体，检查Rider是否为其主人，不是则抖落
        if beefalo.components.rideable:IsSaddled() then
            local __rider = beefalo.components.rideable:GetRider()
            if __rider and __rider.GUID ~= self.inst.GUID and __rider.components.rider then
                __rider.components.rider:ActualDismount()
            end
        end
        self.saved_beefalo = beefalo
        self.beefalo_save = beefalo:GetSaveRecord()
        self.beefalo_saddle_save =
            beefalo.components.rideable:IsSaddled() and beefalo.components.rideable.saddle:GetSaveRecord() or nil
        ...
        if data.isRiding then
            ...
            if beefalo.components.rideable:IsSaddled() then
                self.inst:DoTaskInTime(
                    0,
                    function()
                        self.inst.components.rider:Mount(beefalo, true)
                    end
                )
            else
                self.inst:DoTaskInTime(

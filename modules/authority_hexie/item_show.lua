...
if IsServer then
    ...
    local old_ShowMeHintFunc =
        _G.MOD_RPC_HANDLERS["ShowMeHint"] and _G.MOD_RPC["ShowMeHint"] and _G.MOD_RPC["ShowMeHint"]["Hint"] and
        _G.MOD_RPC_HANDLERS["ShowMeHint"][_G.MOD_RPC["ShowMeHint"]["Hint"].id]
    ...
    if old_ShowMeHintFunc ~= nil then
        _G.MOD_RPC_HANDLERS["ShowMeHint"][_G.MOD_RPC["ShowMeHint"]["Hint"].id] = function(player, guid, item)
            ...
            elseif
                item ~= nil and item.components ~= nil and item.components.worldmigrator ~= nil and
                    item.components.worldmigrator.linkedWorld ~= nil and
                    connect_worlds and
                    connect_worlds[item.components.worldmigrator.linkedWorld] ~= nil and
                    connect_worlds[item.components.worldmigrator.linkedWorld].name ~= nil
             then
                old_ShowMeHintFunc(player, item.GUID, item)
                local old_s = player.player_classified.net_showme_hint:value()
                local old_hint = GetHintValue(old_s, item)
                ...
                if toServerName ~= nil and string.find(old_hint, toServerName) == nil then
                    s =
                        "\n" ..
                        GetSayMsg("item_server_to", formServerName, toServerName) ..
                            (old_hint ~= "" and "\2" .. old_hint or "")
                    if s ~= "" then
                        player.player_classified.net_showme_hint:set(guid .. ";" .. s)
                    end
                elseif old_hint ~= "" then

...
if IsServer then
    if beefalo_power then
        ...
        function Rideable:Buck(gentle, isbuck)
            ...
            if
                self.rider ~= nil and self.rider.components.rider ~= nil and
                    ((obedience == 0 or self.inst.ownerlist == nil) or self.inst.components.health:IsDead() or isbuck)
             then
    ...
    function Teleporter:Activate(doer)
        ...
        --使用了#牛停，则不跟随跳虫洞
        if ret and doer.beefalo_follow then
            ...
            for _, ent in pairs(ents) do
                if
                    ent.components.domesticatable and ent.components.rideable and
                        not ent.components.rideable:IsBeingRidden() and
                        ent.ownerlist ~= nil and
                        ent.ownerlist.master == doer.userid
                 then
                    beefalo = ent
                    break
                end
    ...
    local function CanShareTarget(dude)
        return dude:HasTag("beefalo") and not dude:IsInLimbo() and
            not (dude.components.health:IsDead() or dude:HasTag("player"))
    end
                    ...
                    if data.saddle == nil then
                        if inst.ownerlist ~= nil then
                            ...
                            inst.components.health:StartRegen(
                                TUNING.BEEFALO_HEALTH_REGEN,
                                TUNING.BEEFALO_HEALTH_REGEN_PERIOD
                            )
    ...
    local function PlayerPostInit(inst)
        ...
        function inst._OnDespawn(inst, ...)
            ...
            --如果使用了#牛停，且未在骑行，则不保存牛
            local is_riding = inst.components.rider and inst.components.rider:IsRiding()
            if inst.beefalo_follow or is_riding then

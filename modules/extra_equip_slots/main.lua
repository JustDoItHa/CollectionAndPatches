...
AddComponentPostInit(
        ...
        function(self, _)
            ...
            --                 "equip",
            --                 function(inst, data)
            --                     if data.eslot == setting_render_strategy then
            --                         local inventory = DST and inst.replica.inventory or inst.components.inventory
            --                         if inventory ~= nil then
...
AddClassPostConstruct(
        ...
        function(inst)
            ...
            if checkentity(inst.equipslotinfo) then -- 勋章栏 排序置后
...
if setting_backpack_slot then
    ...
    t.SCENE.repairable = function(inst, doer, actions, right)
        if
        right and (inst:HasTag("repairable_sculpture") or inst:HasTag("repairable_moon_altar")) and doer.replica.inventory ~= nil and
...
if setting_backpack_slot and (IsServer or not DST) then
    ...
    local function bagonequip(inst, owner)
        ...
        if skin_build ~= nil then
            ...
            owner.AnimState:OverrideItemSkinSymbol("swap_body_tall", skin_build, "backpack", inst.GUID, bag_symbol[inst.prefab])
